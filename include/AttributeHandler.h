...
namespace DataStream{
	...
	static void ReadAttribute(hid_t Loc, std::string Path, std::string Name, void *Addr){
		if(!FindAttribute(Loc, Path, Name)) NoAttributeError(Name, Path);

		hid_t Attr = H5Aopen_by_name(Loc, Path.c_str(), Name.c_str(), H5P_DEFAULT, H5P_DEFAULT); 
